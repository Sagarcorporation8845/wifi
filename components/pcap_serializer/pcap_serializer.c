...
uint8_t *pcap_serializer_init() {
    ...
    const pcap_global_header_t pcap_global_header = {
    ...
    if (pcap_buffer == NULL) {
        ESP_LOGE(TAG, "Unable to allocate PCAP global header");
        return NULL;
    }
    ...
    memcpy(pcap_buffer,
           &pcap_global_header,
           sizeof(pcap_global_header_t));

    return pcap_buffer;
}

void pcap_serializer_append_frame(const uint8_t *buffer,
                                  unsigned size,
                                  unsigned ts_usec) {
    if (buffer == NULL || size == 0) {
        ESP_LOGD(TAG, "Empty frame. Not appending anything.");
        return;
    }

    if (pcap_buffer == NULL) {
        ESP_LOGE(TAG, "PCAP buffer is not initialized");
        return;
    }
    ...
    if (reallocated_pcap_buffer == NULL) {
        ESP_LOGE(TAG, "Error reallocating PCAP buffer");
        return;
    }

    memcpy(&reallocated_pcap_buffer[pcap_size],
           &pcap_record_header,
           sizeof(pcap_record_header_t));

    memcpy(&reallocated_pcap_buffer[
               pcap_size + sizeof(pcap_record_header_t)],
           buffer,
           size);
