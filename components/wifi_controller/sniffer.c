...
void wifictl_sniffer_start(uint8_t channel) {
    ...
    /*
     * ESP32 cannot change channel while an associated STA is using the
     * interface. Preserve the original project behaviour, but keep the
     * operation explicit so callers know it can affect the management AP.
     */
    ...
    ESP_ERROR_CHECK(esp_wifi_set_promiscuous_rx_cb(&frame_handler));
}
